use std::{collections::{BinaryHeap, HashMap}, iter::FromIterator, iter::Iterator, i32::MAX,};
use tile::Tile;
use port::Port;
use OrdPosition;
...
impl World {
    ...
    fn reachable(&self, position: Position) -> impl Iterator<Item = &(Position)>  {
        self.map.iter().filter(move |(other_position, tile)| {
            (position.coords.x - other_position.coords.x).abs() +
            (position.coords.y - other_position.coords.y).abs() <= 1 &&
